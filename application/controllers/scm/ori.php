...
class Ori extends CI_Controller {
	...
	public function listInc() {
		...
		$where = 'a.isDelete=0 and a.transType=153401 and a.parent='.$this->parent;
	...
	public function addInc(){
      ...
      $data['created'] = $this->newTime;
			$info = elements(array(
				...
				'totalAmount','buId','billDate','uid','userName','accId','parent','created'),$data,NULL);
			$this->db->trans_begin();
	...
	public function listExp() {
		...
		$beginDate = str_enhtml($this->input->get_post('beginDate',TRUE));
		$endDate   = str_enhtml($this->input->get_post('endDate',TRUE));
		$where = 'a.isDelete=0 and a.transType=153402 and a.parent='.$this->parent;
		$where .= $matchCon  ? ' and a.postData like "%'.$matchCon.'%"' : '';
		$where .= $beginDate ? ' and a.billDate>="'.$beginDate.'"' : '';
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : '';
		$where .= $this->common_model->get_admin_purview();
	...
	public function addExp(){
      ...
      $data['created'] = $this->newTime;
			$info = elements(array(
				...
				'totalAmount','buId','billDate','uid','userName','accId','parent','created'),$data);
			$this->db->trans_begin();
			$iid = $this->mysql_model->insert('invoice',$info);
