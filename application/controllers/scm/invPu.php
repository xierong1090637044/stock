...
class InvPu extends CI_Controller {
	...
	public function purList() {
		...
		$where = 'a.isDelete=0 and a.billType="PUR" and parent='.$this->parent;
		$where .= $transType ? ' and a.transType='.$transType : ' and parent='.$this->parent;
		$where .= $matchCon  ? ' and a.postData like "%'.$matchCon.'%"' : ' and parent='.$this->parent;
		$where .= $beginDate ? ' and a.billDate>="'.$beginDate.'"' : ' and parent='.$this->parent;
		$where .= $endDate   ? ' and a.billDate<="'.$endDate.'"' : ' and parent='.$this->parent;
	...
	public function exportInvPu(){
		...
		$endDate   = str_enhtml($this->input->get_post('endDate',TRUE));
		$order = $sidx ? $sidx.' '.$sord :' a.id desc';
		$where = 'a.isDelete=0 and a.transType='.$transType.' and parent='.$this->parent;
		$where .= $matchCon  ? ' and a.postData like "%'.$matchCon.'%"' : ' and parent='.$this->parent;
		$where .= $beginDate ? ' and a.billDate>="'.$beginDate.'"' : ' and parent='.$this->parent;
		$where .= $endDate ? ' and a.billDate<="'.$endDate.'"' : ' and parent='.$this->parent;
		$where .= $this->common_model->get_admin_purview();
	...
	public function findUnhxList(){
		...
		$where .= $billno ? ' and a.billNo="'.$billno.'"' : ' and parent='.$this->parent;
		$where .= $buid > 0 ? ' and a.buId='.$buid.'' : ' and parent='.$this->parent;
		$where .= strlen($begindate)>0 ? ' and a.billDate>="'.$begindate.'"' : ' and parent='.$this->parent;
		$where .= strlen($enddate)>0 ? ' and a.billDate<="'.$enddate.'"' : ' and parent='.$this->parent;
	...
	public function add(){
      ...
      $data['created'] = $this->newTime;
			$info = elements(array(
				...
				'accId','modifyTime','parent','created'),$data,NULL);
			$this->db->trans_begin();
	...
	private function invoice_info($iid,$data) {
		foreach ($data['entries'] as $arr=>$row) {
			...
			$v[$arr]['deduction']        = $row['deduction'];
			$v[$arr]['description']      = $row['description'];
      $v[$arr]['parent']        = $data['parent'];
			$v[$arr]['created']      = $data['created'];
