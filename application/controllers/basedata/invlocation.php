...
class Invlocation extends CI_Controller {
	...
	public function index(){
		$list = $this->mysql_model->get_results('storage','(isDelete=0) and parent='.$this->parent.' '.$this->common_model->get_location_purview(1).' order by id desc');
    ...
    $data['created'] = $this->newTime;
		if (count($data)>0) {
			...
			$sql  = $this->mysql_model->insert('storage',elements(array('name','locationNo','parent','created'),$data));
			if ($sql) {
				$data['id'] = $sql;
