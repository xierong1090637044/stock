...
class Employee extends CI_Controller {
	...
	public function index(){
		$list = $this->mysql_model->get_results('staff',array('isDelete'=>0,'parent'=>$this->parent),'id desc');
    ...
    $data['created'] = $this->newTime;
		...
		if (count($data)>0) {
			...
			$sql  = $this->mysql_model->insert('staff',elements(array('name','number','parent','created'),$data));
			if ($sql) {
				$data['id'] = $sql;
