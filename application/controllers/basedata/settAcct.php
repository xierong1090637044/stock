...
class Settacct extends CI_Controller {
	...
	public function index(){
		$list = $this->mysql_model->get_results('account',array('isDelete'=>0,'parent'=>$this->parent),'id desc');
    ...
    $data['created'] = $this->newTime;
		if (count($data)>0) {
			...
			$info = elements(array('name','number','amount','date','type','parent','created'),$data);
