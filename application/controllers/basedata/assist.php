...
class Assist extends CI_Controller {
	...
	public function index(){
		...
		$where  = '(isDelete=0) and typeNumber="'.$typeNumber.'" and parent='.$this->parent;
	...
	public function getAssistType(){
	    $v = array(
			...
			4 => array('id'=>1005,'name'=>'支出类别','number'=>'g')
		);
		$json['status'] = 200;
		$json['msg']    = 'success';
		$json['data']['items']      = $v;
    ...
    $data['created'] = $this->newTime;
		...
		$sql  = $this->mysql_model->insert('category',elements(array('name','typeNumber','parent','created'),$data));
		if ($sql) {
    ...
    private function trade_add($data){
		if ($data['parentId']==0) {
		    $data['level'] = 1;
			$newid = $this->mysql_model->insert('category',elements(array('name','typeNumber','level','parentId','parent','created'),$data));
